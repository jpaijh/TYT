...
public class CalendarView extends View implements View.OnTouchListener {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        surface.height = MeasureSpec.getSize(heightMeasureSpec) * 2 / 5;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        Log.d(TAG, "[onLayout] changed:" + (changed ? "new size" : "not change") + " left:" + left + " top:" + top + " right:"
                + right + " bottom:" + bottom);
        if (changed) {
            surface.init();
        }
    ...
    //上一月
    public String clickLeftMonth(Date startDate) {
        if (startDate == null || (startDate != null && curDate.after(startDate))) {
            calendar.setTime(curDate);
            calendar.add(Calendar.MONTH, -1);
    ...
    //下一月
    public String clickRightMonth(Date endDate) {
        if (endDate == null || (endDate != null && curDate.before(endDate))) {
            calendar.setTime(curDate);
            calendar.add(Calendar.MONTH, 1);
            curDate = calendar.getTime();
            invalidate();
